...
export class Game {
    ...
    spawnFruits() {
        ...
        // Shuffle only fruit images
        const shuffledFruitImages = [...this.fruitImages];
        this.shuffleArray(shuffledFruitImages);
    
        // Keep options in original order from the database
        const options = this.currentQuestion.options;
        
        // Create fruits with original options but random images
        this.fruitsInGame = options.map((option, index) => ({
            option,
            index,
            x: 0,
            y: 0,
            isSliced: false,
            image: shuffledFruitImages[index] // Random image but fixed text
        }));
